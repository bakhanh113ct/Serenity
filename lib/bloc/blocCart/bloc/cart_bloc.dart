  ...
  CartBloc() : super(CartInitial()) {
    on<LoadCart>((event, emit) async {
      ...
      final List<Product> listProduct =
          await CartRepository().getProduct(listCategory[0]);
    ...
    on<ChooseCategory>((event, emit) async {
      ...
      final List<Product> listProduct =
          await CartRepository().getProduct(event.newCategory);
      emit(CartLoaded(state.listCategory, listProduct, state.listProductCart,
          event.newCategory));
    });
    on<SearchProduct>((event, emit) async {
      final state = this.state as CartLoaded;
      final List<Product> temp =
          await CartRepository().getProduct(state.selectedCategory);
      final List<Product> listProduct = event.query != ""
          ? temp
    ...
    on<AddProductCart>((event, emit) {
      ...
      if (state.listProductCart.contains(tempProductCart)) {
        ...
        if (temp.amount! + 1 <= int.parse(temp.product!.amount.toString())) {
          listProductCart[state.listProductCart
                  .indexWhere((element) => element.product == event.product)]
              .amount = temp.amount! + 1;
          emit(CartLoaded(state.listCategory, state.listProduct,
              listProductCart, state.selectedCategory));
        } else {
          emit(CartLoaded(state.listCategory, state.listProduct,
              state.listProductCart, state.selectedCategory));
        }
      } else {
        List<ProductCart> listProductCart = state.listProductCart
          ..add(ProductCart(product: event.product, amount: 1));
        emit(CartLoaded(state.listCategory, state.listProduct, listProductCart,
            state.selectedCategory));
      }
    });
