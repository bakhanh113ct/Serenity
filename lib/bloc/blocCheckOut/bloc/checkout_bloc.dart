...
class CheckoutBloc extends Bloc<CheckoutEvent, CheckoutState> {
  CheckoutBloc() : super(CheckoutInitial()) {
    on<LoadCheckout>((event, emit) async {
      ...
      emit(CheckoutLoaded(
          listCustomer, null, listVoucher, null, totalItem, 0, totalItem));
    });
    ...
    on<ChooseVoucher>((event, emit) {
      ...
      emit(CheckoutLoaded(state.listCustomer, state.selectedCustomer,
          state.listVoucher, event.voucher, state.totalItem, discount, total));
    });
